package team9.demo.facade.auth;
...
public class AccountFacade {
    ...
    public void createPassword(UserId userId, String rawPassword, String email, LocalDate birth) {
        String hashedPassword = authService.encryptPassword(rawPassword);
        userService.createPassword(userId, hashedPassword);
        userService.updateBirthdayAndEmail(userId, email, birth); // 생일, 이메일 추가
    }
    ...
    public UserId login(
        ...
        User user = userService.getUserByEmail(email, AccessStatus.ACCESS);
